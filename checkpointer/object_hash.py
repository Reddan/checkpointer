...
import tokenize
import numpy as np
from typing import Any, TypeVar, TypeAliasType, Callable
...
class ObjectHash:
  ...
  def _update_one(self, obj: Any) -> None:
    match obj:
      ...
      case TypeAliasType():
        self.update_hash("TypeAliasType")
        self.update(obj.__name__, obj.__value__)
  ...
  def update_object(self, obj: object) -> None:
    ...
    if hasattr(obj, "__reduce_ex__") or hasattr(obj, "__reduce__"):
      ...
      self.update("slots", {slot: getattr(obj, slot, None) for slot in getattr(obj, "__slots__")})
