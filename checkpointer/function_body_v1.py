"""
Usage in new function_body.py:

  if vals := list({hashing.hash(v): v for vs in depend_values for v in vs}.values()):
    print("Depend values:", vals)
  from .function_body_v1 import get_depend_fns as get_depend_fns_old
  depends_old = get_depend_fns_old(fn)
"""
...
from .utils import unwrap_fn, get_cell_contents
...
def get_referenced_global_names(code: CodeType) -> set[str]:
  # TODO: Fix for module.func syntax
...
def get_global_depends(fn: Callable) -> set[Callable]:
  # can just filter on code.co_consts for depends directly?
